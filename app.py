...
檔案名稱: app.py
套件需求: streamlit, plotly, pandas, numpy, scikit-learn, requests

執行方式:
  1. 確保 Ollama 已開啟並載入模型: ollama run qwen2.5
...
【處置原則】
- 若為 ALERT（重大警告，物理超標），請優先評估休止系統 / 緊急停機或降載急救防護 SOP。
- 若為 WARNING（早期預警，統計偏離），請提供預防性巡檢、潤滑或感測器校正 SOP。

請嚴格輸出 JSON 格式，包含兩個 key：
1. "root_cause": 潛在故障真因分析（簡潔一句話，30字內）
2. "action_suggestion": 現場工程師應採取的具體 SOP 處置動作（50字內）
...
只輸出 JSON，不要加入額外說明："""
    ...
    try:
        ...
        return (
            f"Ollama 連線失敗 (請確認 {ollama_url} 與 ollama run {model_name})",
            "執行基礎巡檢",
        )
...
def run_anomaly_pipeline(
    ...
    for i in range(total_rows):
        ...
        # 動態串流模式下且屬於最新一筆資料時，才呼叫 Ollama LLM
        should_call_llm = enable_llm and is_streaming and (i == total_rows - 1)
        ...
        if has_physical_violation:
            ...
            if should_call_llm:
                ...
                if "過熱" in base_cause:
                    act = "檢修冷卻系統並評估緊急停機/降載"
                elif "震動" in base_cause:
                    act = "暫停機台運轉並檢查主軸軸承"
                else:
                    act = "依規範派員巡檢修復"
...
def main():
    ...
    st.sidebar.caption("Smart Factory Equipment Anomaly Alert Dashboard")
    ...
    ollama_url = st.sidebar.text_input(
        ...
        help="若部署至 Streamlit Cloud，請輸入 ngrok 或 Cloudflare Tunnel 網址 (例: https://xxxx.ngrok-free.app)",
    )

    model_name = st.sidebar.text_input(
        "LLM 模型名稱",
        value="qwen2.5",
        help="請確保 Ollama 地端已載入該模型 (ollama run qwen2.5)",
    )

    enable_llm = st.sidebar.toggle(
        "啟用 Qwen 2.5 動態根因診斷",
        value=True,
        help="當觸發 WARNING 或 ALERT 時，進行 LLM 動態根因與 SOP 推論 (僅在串流模式最新筆觸發)",
    )
    ...
    if not st.session_state.test_executed:
        ...
        with col_act:
            ...
            st.markdown(f"- **LLM 模型**: `{model_name}`")
            if st.button(
        ...
        with col_exp1:
            with st.expander(
                ...
                st.markdown(f"""
                    ...
                    **第二層：統計離群判斷（預警 🟡）**
                    1. Isolation Forest 計算決策分數
                    2. 用穩健統計量 **Median + MAD** 取代 Mean/Std：
                       `Modified Z = 0.6745 × (S - median(S)) / MAD(S)`
                       門檻採用 **Modified Z > {MODIFIED_Z_THRESHOLD}**
                    3. 要求連續 **{DEBOUNCE_WINDOW} 分鐘**呈現離群狀態，且變化速率超出歷史波動
                       **{TREND_Z_THRESHOLD} 個穩健標準差**。
            ...
            with st.expander(
                ...
                st.markdown("""
                    **上傳 CSV 建議欄位名稱與資料型別**：
                    ...
                    - `temp` *(必填)*：設備溫度 (°C)
                    - `pressure` *(必填)*：管線壓力 (bar)
                    - `vibration` *(必填)*：三軸震動 (g)
                    - `label` *(選填)*：實際標籤 (`normal` / `abnormal`)
                """)
    ...
    with st.expander("📄 點此檢視原始 CSV 感測數據 (Current Batch Data)", expanded=False):
    ...
    m2.metric(
        ...
        delta=f"Isolation Forest + {model_name.upper()} LLM",
    )

    st.metric(
        f"當前異常真因 ({model_name} 推論)",
        f"{latest_cause}",
